...
#region Autogenerated code
	public partial class Window : Gtk.Bin {
		...
		{
			GtkWindowClass class_iface = GetClassStruct (gtype, false);
			class_iface.SetFocus = callback;
		...
		{
			SetFocusNativeDelegate unmanaged = GetClassStruct (this.LookupGType ().GetThresholdType (), true).SetFocus;
		...
		{
			GtkWindowClass class_iface = GetClassStruct (gtype, false);
			class_iface.FocusActivated = callback;
			OverrideClassStruct (gtype, class_iface);
		}
		...
		{
			FocusActivatedNativeDelegate unmanaged = GetClassStruct (this.LookupGType ().GetThresholdType (), true).FocusActivated;
			if (unmanaged == null) return;
		...
		{
			GtkWindowClass class_iface = GetClassStruct (gtype, false);
			class_iface.DefaultActivated = callback;
			OverrideClassStruct (gtype, class_iface);
		}
		...
		{
			DefaultActivatedNativeDelegate unmanaged = GetClassStruct (this.LookupGType ().GetThresholdType (), true).DefaultActivated;
			if (unmanaged == null) return;
		...
		struct GtkWindowClass {
			public SetFocusNativeDelegate SetFocus;
			public FocusActivatedNativeDelegate FocusActivated;
			public DefaultActivatedNativeDelegate DefaultActivated;
		...
		static Dictionary<GLib.GType, GtkWindowClass> class_structs;

		static GtkWindowClass GetClassStruct (GLib.GType gtype, bool use_cache)
		{
			if (class_structs == null)
				class_structs = new Dictionary<GLib.GType, GtkWindowClass> ();

			if (use_cache && class_structs.ContainsKey (gtype))
				return class_structs [gtype];
			else {
				IntPtr class_ptr = new IntPtr (gtype.GetClassPtr ().ToInt64 () + class_offset);
				GtkWindowClass class_struct = (GtkWindowClass) Marshal.PtrToStructure (class_ptr, typeof (GtkWindowClass));
				if (use_cache)
		...
		static void OverrideClassStruct (GLib.GType gtype, GtkWindowClass class_struct)
		{
			IntPtr class_ptr = new IntPtr (gtype.GetClassPtr ().ToInt64 () + class_offset);
