...
#region Autogenerated code
	public partial class Settings : GLib.Object {
		...
		[DllImport("libgio-2.0-0.dll", CallingConvention = CallingConvention.Cdecl)]
		static extern IntPtr g_settings_new_with_backend(IntPtr schema, IntPtr backend);
		...
		[DllImport("libgio-2.0-0.dll", CallingConvention = CallingConvention.Cdecl)]
		static extern IntPtr g_settings_new_with_backend_and_path(IntPtr schema, IntPtr backend, IntPtr path);

		public Settings (string schema, GLib.SettingsBackend backend, string path) : base (IntPtr.Zero)
		{
			if (GetType () != typeof (Settings)) {
				var vals = new List<GLib.Value> ();
				var names = new List<string> ();
			...
			Raw = g_settings_new_with_backend_and_path(native_schema, backend == null ? IntPtr.Zero : backend.Handle, native_path);
			GLib.Marshaller.Free (native_schema);
			GLib.Marshaller.Free (native_path);
		}

		[DllImport("libgio-2.0-0.dll", CallingConvention = CallingConvention.Cdecl)]
		static extern IntPtr g_settings_new_with_path(IntPtr schema, IntPtr path);

		public Settings (string schema, string path) : base (IntPtr.Zero)
		{
			if (GetType () != typeof (Settings)) {
				var vals = new List<GLib.Value> ();
				var names = new List<string> ();
			...
			IntPtr native_schema = GLib.Marshaller.StringToPtrGStrdup (schema);
			IntPtr native_path = GLib.Marshaller.StringToPtrGStrdup (path);
			Raw = g_settings_new_with_path(native_schema, native_path);
			GLib.Marshaller.Free (native_schema);
			GLib.Marshaller.Free (native_path);
		}
		...
		{
			GSettingsClass class_iface = GetClassStruct (gtype, false);
			class_iface.WritableChanged = callback;
		...
		{
			WritableChangedNativeDelegate unmanaged = GetClassStruct (this.LookupGType ().GetThresholdType (), true).WritableChanged;
		...
		{
			GSettingsClass class_iface = GetClassStruct (gtype, false);
			class_iface.WritableChangeEvent = callback;
			OverrideClassStruct (gtype, class_iface);
		}
		...
		{
			WritableChangeEventNativeDelegate unmanaged = GetClassStruct (this.LookupGType ().GetThresholdType (), true).WritableChangeEvent;
		...
		{
			GSettingsClass class_iface = GetClassStruct (gtype, false);
			class_iface.ChangeEvent = callback;
			OverrideClassStruct (gtype, class_iface);
		}
		...
		{
			ChangeEventNativeDelegate unmanaged = GetClassStruct (this.LookupGType ().GetThresholdType (), true).ChangeEvent;
		...
		struct GSettingsClass {
			public WritableChangedNativeDelegate WritableChanged;
			public ChangedNativeDelegate Changed;
			public WritableChangeEventNativeDelegate WritableChangeEvent;
			public ChangeEventNativeDelegate ChangeEvent;
			[MarshalAs (UnmanagedType.ByValArray, SizeConst=20)]
			private IntPtr[] Padding;
		}

		static uint class_offset = ((GLib.GType) typeof (GLib.Object)).GetClassSize ();
		static Dictionary<GLib.GType, GSettingsClass> class_structs;

		static GSettingsClass GetClassStruct (GLib.GType gtype, bool use_cache)
		{
			if (class_structs == null)
				class_structs = new Dictionary<GLib.GType, GSettingsClass> ();

			if (use_cache && class_structs.ContainsKey (gtype))
				return class_structs [gtype];
			else {
				IntPtr class_ptr = new IntPtr (gtype.GetClassPtr ().ToInt64 () + class_offset);
				GSettingsClass class_struct = (GSettingsClass) Marshal.PtrToStructure (class_ptr, typeof (GSettingsClass));
				if (use_cache)
		...
		static void OverrideClassStruct (GLib.GType gtype, GSettingsClass class_struct)
		{
			IntPtr class_ptr = new IntPtr (gtype.GetClassPtr ().ToInt64 () + class_offset);
